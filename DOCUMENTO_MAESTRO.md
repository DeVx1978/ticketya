...
29. Compra multi-pasajero e ida y vuelta -- **UNIFICADOS EN UN SOLO TRABAJO DE CONSTRUCCION (hallazgo real, 07-ago-2026), alcance tecnico completo confirmado, construccion pendiente.** El backend NO distingue entre "varios pasajeros, mismo viaje" y "mismo(s) pasajero(s), 2 viajes distintos" -- son la misma capacidad vista desde 2 angulos. Evidencia real: `viajeId` vive DENTRO de cada entrada de `PasajeroCheckout` (no una vez para toda la compra), y se reviso linea por linea toda la validacion de `procesarCompra` (menores de edad, calculo de monto total, aplicacion de creditos) sin encontrar ninguna regla que fuerce a los pasajeros de una misma compra a compartir el mismo viaje -- la unica regla que cruza pasajeros es sobre creditos (deben ser de la misma cooperativa, nada que ver con el viaje). El backend ya esta listo de punta a punta (`checkout.service.ts`, `procesarCompra(pasajeros: PasajeroCheckout[], ...)` recibe un arreglo, el controlador lo pasa completo sin recortar) -- el trabajo real es 100% de frontend: la pantalla de asientos debe permitir elegir asientos de MAS DE UN VIAJE (no solo del mismo, como haria falta solo para multi-pasajero), y el checkout debe pedir los datos de cada pasajero elegido, cada uno con su propio `viajeId`. El bloqueo temporal de asientos (`bloquearAsiento`) ya soporta re-bloquear el propio asiento sin fallar, renovando su tiempo de expiracion -- resuelve el riesgo de que un asiento elegido primero expire mientras se completan los demas, sin tocar el backend. Construir esto bien (con soporte real para varios viajes) da ambas funciones -- multi-pasajero e ida y vuelta -- en un solo trabajo, en vez de construirlas 2 veces por separado.
...
31. Compra como invitado -- pendiente, prioridad mas baja
