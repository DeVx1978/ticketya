...
**2. Nombres/apellidos del pasajero sin validación real (PR #117).** El checkout aceptaba `"juan Carlos López Gómez"` -- minúscula al inicio, sin ningún control de formato (antes solo `@IsString() @MinLength(2)`). Corregido en `crear-compra.dto.ts`: `PATRON_NOMBRE_REAL` exige solo letras (tildes y ñ incluidas), y `@Transform` capitaliza cada palabra automáticamente antes de guardar (el `ValidationPipe` global ya tenía `transform: true`).
  ...
  **Confirmado con prueba real, 19-ago-2026:** el director escribió deliberadamente `"maria jose"` / `"gonzalez pino"` en minúsculas en el formulario, completó la compra, y la pantalla final de confirmación mostró `"Maria Jose Gonzalez Pino"` -- capitalización correcta en las 4 palabras. La capitalización automática funciona en producción. (Nota aparte, no es un error: no aparece la tilde en "María" porque el director no la escribió al teclear -- el sistema corrige mayúsculas, no inventa acentos.)
...
- **Footer incompleto** -- investigación real comparativa (Expedia, HubSpot, guía de señales de confianza para turismo 2026) confirma 3 huecos reales: (1) `configuracion_plataforma.soporte_correo`/`soporte_telefono` ya existen y se usan en el PDF del boleto, pero nunca se muestran en el footer del sitio web; (2) no existe ninguna página de Términos de Servicio ni Política de Privacidad; (3) el footer es una sola fila plana (logo + 2 enlaces), sin estructura de columnas como usan las plataformas de referencia. El director decidió dejarlo para el día siguiente -- las páginas legales en particular requieren criterio legal, no deben redactarse sin confirmación explícita del alcance.
...
## 5.68 Bloque de bienvenida del vendedor: 4 botones duplicaban el menú -- reemplazado por "Viajes de hoy" real -- 25-ago-2026

El director probó el fix de la sección 5.67 y encontró un hallazgo real de diseño: el bloque de bienvenida del vendedor mostraba 4 botones (Ver rutas, Ver unidades, Ver viajes, Validar boleto) que eran exactamente los mismos 4 enlaces ya visibles arriba, en el menú lateral -- solo repetidos con otro texto, sin aportar nada.

**2 opciones presentadas, el director eligió la B:** quitar los botones y dejar solo texto (A), o reemplazarlos por contenido real y distinto (B) -- investigado antes de proponerla: confirmado que `GET /coop/viajes` ya es accesible para `vendedor` (`@Roles('admin_cooperativa', 'vendedor')`, mismo endpoint que ya usa la sección "Viajes" del menú), y que `fechaSalida` es una columna `date` real (sin componente de hora ni huso horario -- a diferencia de `horaSalidaProgramada`/`horaLlegadaEstimada`, que sí tienen el gotcha real de zona horaria documentado en `ENTREGA_TECNICA_EXHAUSTIVA.md` sección 3), así que filtrar por el string de fecha de hoy, sin conversión, es seguro.

**Construido (rama `feat/vendedor-viajes-de-hoy`, un solo archivo, `apps/web/app/panel-empresa/page.tsx`):** en el caso del vendedor, en vez de solo retornar del `useEffect`, ahora llama a `listarViajesCoop` (mismo cliente real ya existente) y filtra por `fechaSalida === hoy` (calculado con `toLocaleDateString("sv-SE")`, truco real para obtener `YYYY-MM-DD` de la fecha local del navegador, mismo formato que devuelve la columna `date`). El bloque visual con los 4 botones se reemplazó por una tabla real de solo lectura (Hora, Ruta, Unidad), usando los mismos campos reales que ya trae `ViajeCoopResumen`. El import de `Link` se quitó, ya no se usaba en ningún lado del archivo.

**Verificado con captura real:** ruta temporal con 3 viajes simulados, confirmando formato de hora en español (AM/PM), tabla limpia y consistente con el resto del sitio -- las horas de la captura salen en UTC porque este entorno de verificación corre en ese huso, no en hora de Ecuador; en el navegador real del vendedor, `toLocaleTimeString` convierte correctamente a su hora local. `tsc --noEmit` y `next build` limpios. Cambio 100% frontend, cero archivos tocados en `apps/api`.

## 5.69 Rediseño de `/panel-empresa` -- Fase 1 (TailAdmin SaaS, acento ámbar) -- 25-ago-2026

Orden del director: mismo tratamiento visual real ya aplicado a `/admin` (sección 5.63, TailAdmin), extendido a `/panel-empresa` -- que seguía con el diseño viejo (sin logo, texto plano, enlaces amontonados). Aclaración real importante agregada por el director antes de construir: los 2 paneles deben distinguirse a simple vista, sin depender de leer texto.

**Investigación real hecha antes de proponer el plan:**
- **Variante CRM de TailAdmin, descartada con evidencia real** (`web_fetch` del demo real, `demo.tailadmin.com/crm`): enfocada en "deals"/pipeline de ventas/clientes (Revenue Total, Closed Deals, Sales Category) -- no aplica a gestión de rutas/flota/personal. Se recomendó mantener la variante **SaaS** (misma que admin), correcta conceptualmente para ambos.
- **Acento distintivo, sin inventar un color nuevo:** revisados todos los tokens reales de marca (`globals.css`) -- solo existen `brand-cobalto` (ya usado en admin) y `brand-amber` (`#f5a800`, el color real de la carrocería del bus/logo). Se propuso ámbar para `/panel-empresa` (verificado por el director que el hex coincide exacto con el real) -- decisión con sentido narrativo real: cobalto = infraestructura de toda la plataforma (admin), ámbar = identidad operativa real de Columbus (cooperativa). Estado activo del menú en ámbar **suave** (`bg-brand-amber/15`), no sólido, para no confundirse con los botones de acción reales (que sí son ámbar sólido en todo el sitio).
- **Alcance real:** 11 secciones (`layout.tsx` + 10 páginas) es demasiado para una sola sesión -- se acordó Fase 1 (layout + Panel + Viajes, esta sesión) y Fase 2 (las 8 secciones restantes, sesión aparte).

**Construido (rama `feat/panel-empresa-tailadmin-fase1`, 3 archivos):**
- `layout.tsx`: reescrito por completo -- sidebar fijo en desktop con logo sobre fondo **ámbar sólido** (a diferencia del cobalto de admin, distinguible al instante), 10 enlaces reales con ícono SVG propio por enlace (nuevos, ninguno reutilizado de admin -- conceptos distintos: Rutas, Unidades, Personal, Viajes, Validar boleto, Pagos pendientes, Facturas, Configuración, Carga masiva), menú deslizable equivalente en móvil. Misma lógica real de guardia de sesión y banner de datos legales, sin tocar.
- `page.tsx` (Panel): componente `TarjetaMetrica` con ícono en insignia ámbar suave, mismo patrón que admin. Los 2 botones de "Guardar" corregidos de `bg-brand` (casi negro) a `bg-brand-amber`.
- `viajes/page.tsx`: **hallazgo real concreto señalado por el director** -- las 4 acciones por viaje (Ver pasajeros, Editar, Cambiar unidad, Cancelar viaje) vivían siempre visibles, amontonadas en una columna angosta. Reemplazadas por un menú "···" desplegable (mismo patrón real de acciones de fila de TailAdmin), con cierre real al hacer clic fuera (`useRef` + listener de `mousedown`). Los 3 componentes reales de acción (`BotonEditarViaje`, `BotonCambiarUnidad`, `BotonCancelarViaje`) se reutilizaron tal cual adentro del menú nuevo, sin tocar su lógica interna -- cuando alguno se expande a su formulario en línea, el menú se queda abierto para poder interactuar.

**Verificado con evidencia real, no solo visual:** ruta temporal con datos simulados, capturas en desktop y móvil. Además, **prueba funcional real** con Playwright: clic real en el botón "···" confirmando que el menú se abre (con las 4 acciones visibles), y clic fuera confirmando que se cierra (`0` coincidencias de "Ver pasajeros" tras el clic externo) -- no solo que se ve bien, sino que el mecanismo funciona de punta a punta. `next build` completo confirmando que las 11 rutas reales de `/panel-empresa` siguen compilando con el layout nuevo. `tsc --noEmit` limpio. Cambio 100% frontend, cero archivos tocados en `apps/api`.
