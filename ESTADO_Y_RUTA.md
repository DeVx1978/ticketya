...
- [ ] Selección de asientos con bloqueo temporal (RF-SEAT) — siguiente a
      construir. Requiere resolver primero el patrón de "transacción con
      SET LOCAL por cooperativa" para que las ESCRITURAS también respeten
      RLS correctamente (las lecturas de búsqueda ya lo resuelven vía
      bypass; escribir un asiento SÍ pertenece a una cooperativa
      específica y no debe usar bypass).
- [ ] Checkout y pasajeros (RF-CHECK) — pendiente. La integración de pago
      real con PayPhone no puede completarse todavía: faltan credenciales
      reales (cotización pendiente, ya señalada en el SRS). Se construirá
      con un simulador de pago detrás de una interfaz intercambiable,
      dejado explícito en el código que no es la integración real.
- [ ] Boleto digital con QR (RF-TICKET) — pendiente, depende de checkout.

### ⚠️ Acción pendiente DEL DIRECTOR (no de Claude) antes de seguir instalando

El paquete `TicketYa_Fase2_Busqueda.zip` ya entregado **no va a correr en
tu PC** hasta que hagas esto, una sola vez:

1. En pgAdmin, abrir el Query Tool sobre la base `ticketya`, cargar el
   archivo `packages/db/migrations/manual/004_habilitar_login_roles.sql`.
2. **Antes de ejecutarlo**, editarlo: reemplazar
   `cambia_esta_contrasena_app` y `cambia_esta_contrasena_admin` por dos
   contraseñas propias (anotarlas — se necesitan para el `.env`).
3. Ejecutarlo.
4. Actualizar `apps/api/.env` con las nuevas credenciales (`DATABASE_URL`
   usando `ticketya_app` + su contraseña, `DATABASE_URL_PUBLICO` usando
   `ticketya_platform_admin` + su contraseña) — pendiente de hacer con
   Claude en la próxima sesión si no se hizo antes.

### 📍 Punto exacto de pausa

Backend con autenticación y búsqueda de rutas construidos y verificados
con Postgres real (en el entorno de Claude). **Todavía no confirmado que
corran en la PC del director** con el nuevo esquema de roles — ver acción
pendiente arriba. Después de confirmar eso, el siguiente bloque de
trabajo es RF-SEAT (selección de asientos con bloqueo temporal).
